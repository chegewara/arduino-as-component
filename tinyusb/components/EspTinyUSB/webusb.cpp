...
bool WebUSB::begin()
{
    String url = String("www.tinyusb.org/examples/webusb-serial"); // private
    landingPageURI(url, true);
    USBdevice.enableVENDOR = true;
...
{
    ESP_LOGI(__func__, "");
    uint32_t sent = tud_vendor_write(buffer, size);
    return sent;
}
...
void WebUSB::landingPageURI(String url, bool ssl)
{
    ...
    uint8_t size = url.length();
    _url = (uint8_t*)calloc(1, size + 3);
    memcpy(&_url[3], url.c_str(), size);
    _url[0] = size + 3;
    _url[1] = 3;
    _url[2] = ssl;
}
